...
api = Api(
    ...
    doc='/swagger'  # Swagger UI访问路径
)

# 创建命名空间
face_ns = Namespace('face', description='人脸识别相关操作')
behavior_ns = Namespace('behavior', description='行为检测相关操作')
video_ns = Namespace('video', description='视频流相关操作')
danger_ns = Namespace('danger', description='危险区域检测相关操作')
anomaly_ns = Namespace('anomaly', description='异常事件相关操作')
...
# 人脸注册接口 - 修改为Swagger文档化版本
@face_ns.route('/register')
class FaceRegister(Resource):
    @face_ns.doc('register_face')
    @face_ns.expect(face_ns.model('FaceRegister', {
        'student_id': fields.String(required=True, description='学生ID'),
        'image': fields.String(required=True, description='Base64编码的人脸图像')
    }))
    @face_ns.response(200, '人脸注册成功')
    @face_ns.response(400, '未检测到人脸或多个人脸')
    def post(self):
...
# 删除人脸特征接口 - 修改为Swagger文档化版本
@face_ns.route('/delete/<student_id>')
@face_ns.param('student_id', '学生ID')
class FaceDelete(Resource):
    @face_ns.doc('delete_face')
    @face_ns.response(200, '人脸删除成功')
    @face_ns.response(404, '人脸未找到')
...
# 添加新的综合检测端点
# 视频流接口 - 修改为Swagger文档化版本
@video_ns.route('/feed/<stream_id>')
@video_ns.param('stream_id', '视频流ID')
class VideoFeed(Resource):
    @video_ns.doc('get_video_feed')
    @video_ns.response(200, '视频流获取成功')
    @video_ns.response(400, '无效的视频流ID')
    @video_ns.response(500, '视频流处理失败')
...
# 行为检测视频流接口 - 修改为Swagger文档化版本
@video_ns.route('/behavior_feed/<stream_id>')
@video_ns.param('stream_id', '视频流ID')
class BehaviorFeed(Resource):
    @video_ns.doc('get_behavior_feed')
    @video_ns.response(200, '行为检测视频流获取成功')
    @video_ns.response(400, '无效的视频流ID')
    @video_ns.response(500, '视频流处理失败')
    def get(self, stream_id):
     """行为检测视频流"""
...
# 健康检查接口 - 修改为Swagger文档化版本
@api.route('/health')
class HealthCheck(Resource):
    @api.doc('health_check')
    @api.response(200, '服务运行正常')
    def get(self):
     return jsonify({'status': 'healthy', 'message': 'Face recognition service is running'})


# 测试视频流接口 - 修改为Swagger文档化版本
@video_ns.route('/test/<stream_id>')
@video_ns.param('stream_id', '视频流ID')
class TestStream(Resource):
    @video_ns.doc('test_stream')
    @video_ns.response(200, '视频流测试成功')
    @video_ns.response(400, '无效的视频流ID')
    @video_ns.response(500, '视频流测试失败')
    def get(self, stream_id):
     """测试视频流连接"""
     try:
         # 确保stream_id是数字
...
# 行为类别接口 - 修改为Swagger文档化版本
@behavior_ns.route('/classes')
class BehaviorClasses(Resource):
    @behavior_ns.doc('get_behavior_classes')
    @behavior_ns.response(200, '获取行为类别成功')
    def get(self):
     """获取支持的行为类别"""
     return jsonify({
...
# 行为检测接口 - 修改为Swagger文档化版本
@behavior_ns.route('/detect')
class BehaviorDetection(Resource):
    @behavior_ns.doc('detect_behavior')
    @behavior_ns.expect(behavior_ns.model('BehaviorDetection', {
        'image': fields.String(required=True, description='Base64编码的图像')
    }))
    @behavior_ns.response(200, '行为检测成功')
    @behavior_ns.response(500, '行为检测失败')
    def post(self):
     """单张图片行为检测"""
     try:
         data = request.get_json()
...
# 异常事件接口 - 修改为Swagger文档化版本
@anomaly_ns.route('/events')
class AnomalyEvents(Resource):
    @anomaly_ns.doc('get_anomaly_events')
    @anomaly_ns.param('page', '页码', type='int')
    @anomaly_ns.param('per_page', '每页数量', type='int')
    @anomaly_ns.param('status', '状态筛选')
    @anomaly_ns.response(200, '获取异常事件成功')
    @anomaly_ns.response(500, '获取异常事件失败')
    def get(self):
     """获取异常事件列表"""
     try:
...
# 处理异常事件接口 - 修改为Swagger文档化版本
@anomaly_ns.route('/events/<event_id>/handle')
@anomaly_ns.param('event_id', '事件ID')
class HandleAnomalyEvent(Resource):
    @anomaly_ns.doc('handle_anomaly_event')
    @anomaly_ns.expect(anomaly_ns.model('HandleEvent', {
        'handler': fields.String(description='处理人')
    }))
    @anomaly_ns.response(200, '事件处理成功')
    @anomaly_ns.response(500, '处理事件失败')
    def post(self, event_id):
        ...
        try:
            ...
            return {'error': f'处理事件失败: {str(e)}'}, 500



# 获取异常事件视频接口 - 修改为Swagger文档化版本
@anomaly_ns.route('/events/<event_id>/video')
@anomaly_ns.param('event_id', '事件ID')
class AnomalyEventVideo(Resource):
    @anomaly_ns.doc('get_anomaly_video')
    @anomaly_ns.response(200, '获取视频成功')
    @anomaly_ns.response(404, '视频文件不存在')
    @anomaly_ns.response(500, '获取视频失败')
    def get(self, event_id):
        ...
        try:
            ...
            return {'error': f'获取视频失败: {str(e)}'}, 500


# 综合检测视频流接口 - 修改为Swagger文档化版本
@video_ns.route('/combined_feed/<stream_id>')
@video_ns.param('stream_id', '视频流ID')
class CombinedFeed(Resource):
    @video_ns.doc('get_combined_feed')
    @video_ns.response(200, '综合检测视频流获取成功')
    @video_ns.response(400, '无效的视频流ID')
    @video_ns.response(500, '视频流处理失败')
    def get(self, stream_id):
        ...
        try:
            if not stream_id.isdigit():
                return {'error': 'Invalid stream ID, must be a number'}, 400

            stream_url = f'rtmp://116.205.102.242:9090/live/{stream_id}'
            return Response(gen_frames_with_anomaly_detection(stream_url, mode='combined'),
                          mimetype='multipart/x-mixed-replace; boundary=frame')
        except Exception as e:
            return {'error': 'Failed to process combined video stream'}, 500
...
# 危险区域配置接口 - 修改为Swagger文档化版本
@danger_ns.route('/zones/<stream_id>')
@danger_ns.param('stream_id', '视频流ID')
class DangerZones(Resource):
    @danger_ns.doc('get_danger_zones')
    @danger_ns.response(200, '获取危险区域配置成功')
    def get(self, stream_id):
        """获取指定摄像头的危险区域配置"""
        return danger_zones.get(stream_id, {
            'enabled': False,
            'zones': [],
...
# 设置危险区域配置接口 - 修改为Swagger文档化版本
@danger_ns.doc('set_danger_zone_config')
@danger_ns.expect(danger_ns.model('DangerZoneConfig', {
    'enabled': fields.Boolean(description='是否启用'),
    'zones': fields.List(fields.Nested(danger_ns.model('Zone', {
        'name': fields.String(description='区域名称'),
        'polygon': fields.List(fields.List(fields.Integer, description='坐标点[x,y]'))
    }))),
    'safety_distance': fields.Float(description='安全距离'),
    'stay_time': fields.Float(description='停留时间')
}))
@danger_ns.response(200, '危险区域配置保存成功')
@danger_ns.response(500, '保存失败')
def post(self, stream_id):
    ...
    try:
        ...
        save_danger_zones()
        return {'message': '危险区域配置保存成功'}
    except Exception as e:
        return {'error': f'保存失败: {str(e)}'}, 500


# 切换危险区域检测状态接口 - 修改为Swagger文档化版本
@danger_ns.route('/zones/<stream_id>/toggle')
@danger_ns.param('stream_id', '视频流ID')
class ToggleDangerZone(Resource):
    @danger_ns.doc('toggle_danger_zone')
    @danger_ns.expect(danger_ns.model('ToggleConfig', {
        'enabled': fields.Boolean(required=True, description='是否启用')
    }))
    @danger_ns.response(200, '操作成功')
    @danger_ns.response(500, '操作失败')
    def post(self, stream_id):
        """启用/禁用指定摄像头的危险区域检测"""
        try:
            data = request.get_json()
            ...
            save_danger_zones()
            return {'message': f'危险区域检测已{"启用" if enabled else "禁用"}'}
        except Exception as e:
            return {'error': f'操作失败: {str(e)}'}, 500


# 危险告警接口 - 修改为Swagger文档化版本
@danger_ns.route('/alerts/<stream_id>')
@danger_ns.param('stream_id', '视频流ID')
class DangerAlerts(Resource):
    @danger_ns.doc('get_danger_alerts')
    @danger_ns.response(200, '获取告警成功')
    @danger_ns.response(500, '获取告警失败')
    def get(self, stream_id):
        """获取指定摄像头的危险告警"""
        try:
            stream_alerts = [alert for alert in danger_alerts if alert['stream_id'] == stream_id]
            return {
                'alerts': stream_alerts[-50:],
                'total': len(stream_alerts)
            }
        except Exception as e:
            return {'error': f'获取告警失败: {str(e)}'}, 500


# 清除危险告警接口 - 修改为Swagger文档化版本
@danger_ns.route('/alerts/<stream_id>/clear')
@danger_ns.param('stream_id', '视频流ID')
class ClearDangerAlerts(Resource):
    @danger_ns.doc('clear_danger_alerts')
    @danger_ns.response(200, '告警已清除')
    @danger_ns.response(500, '清除告警失败')
    def post(self, stream_id):
        """清除指定摄像头的告警"""
        try:
            ...
            return {'error': f'清除告警失败: {str(e)}'}, 500



# 危险区域检测视频流接口 - 修改为Swagger文档化版本
@danger_ns.route('/feed/<stream_id>')
@danger_ns.param('stream_id', '视频流ID')
class DangerFeed(Resource):
    @danger_ns.doc('get_danger_feed')
    @danger_ns.response(200, '危险区域检测视频流获取成功')
    @danger_ns.response(400, '无效的视频流ID')
    @danger_ns.response(500, '视频流处理失败')
    def get(self, stream_id):
        """危险区域检测视频流"""
        try:
            if not stream_id.isdigit():
            ...
            return {'error': 'Failed to process danger detection video stream'}, 500

# 添加所有命名空间到API
api.add_namespace(face_ns)
api.add_namespace(behavior_ns)
api.add_namespace(video_ns)
api.add_namespace(danger_ns)
