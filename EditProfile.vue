  ...
  <div class="edit-profile-container">
    <div class="drop-zone" @drop.prevent="handleDrop" @dragover.prevent>
      ...
      <div class="preview-area">
        <img :src="currentPhoto" class="preview-image" />
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      showSaveModal: false,
      selectedFile: null,
      currentPhoto: 'https://via.placeholder.com/200',
      originalData: null,
      fieldModified: {
        name: false,
        gender: false,
    ...
    async loadPersonData() {
      ...
      try {
        ...
        if (result.data.photo_path) {
          this.currentPhoto = `http://localhost:3000${result.data.photo_path}`
        }
    ...
    async saveChanges() {
      ...
      try {
        ...
        if (!updateResponse.ok) {
          throw new Error('基本信息更新失败')
        }

        const updateResult = await updateResponse.json()

        if (!updateResult.success) {
          throw new Error(updateResult.message || '基本信息更新失败')
        }
        ...
        if (this.selectedFile) {
          ...
          const photoResponse = await fetch(`http://localhost:3000/api/user-data/user/${this.id}/photo`, {
            method: 'POST',
            body: formData
          })
          ...
          if (!photoResult.success) {
            throw new Error(photoResult.message || '照片上传失败')
          }
        }
