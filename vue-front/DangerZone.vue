  ...
  <div v-if="!isDrawing && config.zones.length > 0" class="zones-overlay">
    <svg class="zones-preview">
      ...
      <div class="control-panel">
        ...
        <!-- 告警信息 -->
        <div class="panel-section alerts-section">
          <h3>告警信息</h3>
          <div class="alerts-container">
            ...
            <div class="alerts-list" style="max-height: 300px; min-height: 200px;">
...
export default {
  ...
  // 确保只处理直接点击video-stream的事件
  if (!event.target.classList.contains('video-stream')) {
    return;
  }
  ...
  // 防止重复点击（添加防抖）
  if (this.lastClickTime && Date.now() - this.lastClickTime < 200) {
    return;
  }
  ...
  if (x >= 0 && x < rect.width && y >= 0 && y < rect.height) {
    ...
    async loadAlerts() {
      try {
        ...
        // 强制重新计算滚动条
    this.$nextTick(() => {
      const alertsList = this.$el.querySelector('.alerts-list');
      if (alertsList) {
        alertsList.style.overflow = 'hidden';
        alertsList.offsetHeight; // 触发重排
        alertsList.style.overflow = 'auto';
      }
    });
      } catch (error) {
